...
import {
  ...
  FILTER_DECORATOR_NAME_METADATA_KEY,
...
export const applyFilterParameter = (args: any[], target, property: string) => {
  const filterArgIndex = args.findIndex(
    (x) => x?._name_ === FILTER_DECORATOR_NAME_METADATA_KEY
  );
  if (filterArgIndex != -1) {
      ...
      qb.andWhere(
        new Brackets((andBracketsQb) => {
          for (const op of parameters?.and) {
            const andParameters = recursivelyTransformComparators(
              op,
              customFields,
              options?.sqlAlias
            );
      ...
      qb.orWhere(
        new Brackets((orBracketsQb) => {
          for (const op of parameters?.or) {
            const orParameters = recursivelyTransformComparators(
              op,
              customFields,
              options?.sqlAlias
            );
