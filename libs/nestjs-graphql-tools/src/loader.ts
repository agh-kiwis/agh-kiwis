...
import {
  GRAPHQL_SORTING_DECORATOR_METADATA_KEY,
  applySortingParameter,
} from './sorting';
...
export const GraphqlLoader = (args?: GraphqlLoaderOptions) => {
  const options = {
    foreignKey: 'id',
    ...args,
  };
  ...
  return (target, property, descriptor) => {
    ...
    descriptor.value = function (...args) {
      ...
      const loader = args.find(
        (x) => x?._name_ === LOADER_DECORATOR_NAME_METADATA_KEY
      ) as LoaderData<any, any> | PolymorphicLoaderData<any, any, any>;
      const loaderKey = `${concatPath(loader.info.path)}.${
        target.constructor.name
      }.${property}`;
      if (!loader || !loader.parent) {
        throw new Error(
          '@Loader parameter decorator is not first parameter or missing'
        );
      }
      ...
      if (!loader.req._loader[loaderKey]) {
        loader.req._loader[loaderKey] = new DataLoader(async (ids) => {
          if (options.polymorphic) {
            ...
            polyLoader.polimorphicTypes = Object.entries(gs).reduce(
              (acc, [descriminator, entities]) => {
                acc.push({
                  descriminator,
      ...
      if (options.polymorphic) {
        if (
          loader.parent[options.polymorphic.idField] &&
          loader.parent[options.polymorphic.typeField]
        ) {
          ...
          throw new Error(
            `Polymorphic relation Error: Your parent model must provide ${options.polymorphic.idField} and ${options.polymorphic.typeField}`
          );
        }
      } else {
        if (options.accessor) {
        ...
        if (loader.parent[options.foreignKey]) {
          return loader.req._loader[loaderKey].load(
            loader.parent[options.foreignKey]
          );
        }
      }
    };
...
export const mapOneToManyPolymorphicRelation = (
  ...
  const res = typeIds.map(
    (type) => gs[`${type.descriminator}_${type.id}`] || null
  );
  return res;
};
...
const concatPath = (path: any, acc?: string) => {
  if (
    path.typename !== 'Query' &&
    path.typename !== 'Subscription' &&
    path.typename !== 'Mutation'
  ) {
