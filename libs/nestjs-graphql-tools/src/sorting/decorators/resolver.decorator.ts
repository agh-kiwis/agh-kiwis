...
import {
  GraphqlSortingFieldMetadata,
  GraphqlSortingTypeDecoratorMetadata,
...
export const Sorting = (
  ...
  return (target, propertyName, paramIndex) => {
    const name = `${standardize(target.constructor.name)}_${standardize(
      propertyName
    )}`;
    ...
    const customFields = typeFunctions.reduce((acc, typeFn) => {
      const customSortingData: GraphqlSortingTypeDecoratorMetadata =
        Reflect.getMetadata(
          SORTING_DECORATOR_CUSTOM_FIELDS_METADATA_KEY,
          typeFn.prototype
        );
