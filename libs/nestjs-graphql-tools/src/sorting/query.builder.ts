import {
  SORTING_DECORATOR_CUSTOM_FIELDS_METADATA_KEY,
  SORTING_DECORATOR_NAME_METADATA_KEY,
  SORTING_DECORATOR_OPTIONS_METADATA_KEY,
} from './constants';
...
export const applySortingParameter = (
  args: any[],
  target,
  property: string
) => {
  const sortingArgIndex = args.findIndex(
    (x) =>
      Array.isArray(x) &&
      x?.some((x) => x._name_ === SORTING_DECORATOR_NAME_METADATA_KEY)
  );
  if (sortingArgIndex != -1) {
    const options = Reflect.getMetadata(
      ...
      target,
      property
    ) as ISortingDecoratorParams;
    const customFields = Reflect.getMetadata(
      SORTING_DECORATOR_CUSTOM_FIELDS_METADATA_KEY,
      target,
      property
    ) as Map<string, GraphqlSortingFieldMetadata>;
    args[sortingArgIndex] = convertParameters(
      args[sortingArgIndex],
      customFields,
      options
    );
  }
...
const convertParameters = <T>(
  parameters: (SortArgs<T> & ISortingMetadata)[],
